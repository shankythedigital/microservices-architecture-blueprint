...
# ============================================================
# 🚀 Run Asset Service with Tesseract OCR Support
# ============================================================
# This script ensures Tesseract native libraries are accessible
...
# Set library path for native libraries (macOS)
...
cd "$(dirname "$0")"
    ...
    # CRITICAL: On macOS, DYLD_LIBRARY_PATH must be set before JVM starts
    # Maven's spring-boot:run doesn't always pass environment variables correctly
    # So we need to ensure they're set in the current shell environment
    
    # Export environment variables BEFORE running Maven
    export TESSDATA_PREFIX
    if [ -n "$DYLD_LIBRARY_PATH" ]; then
        export DYLD_LIBRARY_PATH
        echo "📝 DYLD_LIBRARY_PATH exported: $DYLD_LIBRARY_PATH"
    fi
    
    # Build JAVA_OPTS with library path
    if [ -n "$JAVA_LIB_PATH" ]; then
        JAVA_OPTS="-Djava.library.path=$JAVA_LIB_PATH"
        ...
        # Still set TESSDATA_PREFIX even if no library path
        if [ -n "$TESSDATA_PREFIX" ]; then
            export MAVEN_OPTS="-DTESSDATA_PREFIX=$TESSDATA_PREFIX"
            echo "📝 Using MAVEN_OPTS: $MAVEN_OPTS"
        fi
    fi
